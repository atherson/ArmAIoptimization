...
struct BenchmarkResult {
    ...
    double peak_memory_mb;          // BUG 8a FIXED: was `size_t`, causing truncation
...
struct BenchmarkSummary {
    ...
    double ttft_mean() const { return mean(ttft_ms_v()); }
    double ttft_p50()  const { return percentile(ttft_ms_v(), 50); }
    double ttft_p95()  const { return percentile(ttft_ms_v(), 95); }
    double ttft_p99()  const { return percentile(ttft_ms_v(), 99); }

    double tps_mean() const { return mean(tps_v()); }
    double tps_p50()  const { return percentile(tps_v(), 50); }
    double tps_p95()  const { return percentile(tps_v(), 95); }

    double memory_mean() const { return mean(mem_v()); }

private:
    std::vector<double> ttft_ms_v() const {
        std::vector<double> v; v.reserve(runs.size());
        for (const auto& r : runs) v.push_back(r.ttft_ms);
        return v;
    }
    std::vector<double> tps_v() const {
        std::vector<double> v; v.reserve(runs.size());
        for (const auto& r : runs) v.push_back(r.tokens_per_second);
        return v;
    }
    std::vector<double> mem_v() const {
        std::vector<double> v; v.reserve(runs.size());
...
public:
    ...
    BenchmarkSummary run() {
        ...
        std::cout << "Runs:       " << config_.n_benchmark_runs
                  << " (warmup: " << config_.n_warmup_runs << ")" << std::endl;
        ...
        // BUG 8b FIXED: load_model was called as (model_path, n_ctx, n_threads)
        //   which matches the 3-argument overload with n_gpu_layers defaulting to 0.
        //   That is fine, but n_ctx and n_threads were swapped in the original call
        //   relative to the header's parameter order.  Fixed here.
        ...
        std::cout << "Model loaded: " << engine.get_model_name() << std::endl << std::endl;
        ...
        size_t total_runs = static_cast<size_t>(config_.n_benchmark_runs) * config_.prompts.size();
        size_t run_count  = 0;
...
private:
    BenchmarkResult run_single(InferenceEngine& engine,
        ...
        result.ttft_ms              = static_cast<double>(response.stats.time_to_first_token.count()) / 1000.0;
        ...
        result.peak_memory_mb       = static_cast<double>(response.stats.peak_memory_bytes) / (1024.0 * 1024.0);
        result.total_time_ms        = static_cast<double>(response.stats.total_inference_time.count()) / 1000.0;

        if (!silent && config_.verbose) {
            std::string prompt_preview    = prompt.size() > 40
                                                ? prompt.substr(0, 40) + "..."
                                                : prompt;
            // BUG 8c FIXED: response.choices[0].message accessed without checking
            //   choices is non-empty, causing UB if generation failed.
            std::string response_preview  = response.choices.empty()
                                                ? "(no response)"
                                                : (response.choices[0].message.size() > 80
                                                       ? response.choices[0].message.substr(0, 80) + "..."
                                                       : response.choices[0].message);

            std::cout << "\n  Prompt:          " << prompt_preview              << std::endl;
            std::cout << "  Response:        " << response_preview              << std::endl;
            std::cout << "  TTFT:            " << std::fixed << std::setprecision(2)
                                               << result.ttft_ms                << " ms"    << std::endl;
            ...
            std::cout << "  Gen tokens:      " << result.generated_tokens                   << std::endl;
            std::cout << "  Peak memory:     " << std::setprecision(2)
                                               << result.peak_memory_mb         << " MB"    << std::endl;
            std::cout << std::endl;
        }
...
void print_summary(const BenchmarkSummary& summary) {
    ...
    std::cout << "Runs: " << summary.runs.size() << std::endl << std::endl;

    std::cout << "Time to First Token (TTFT):" << std::endl;
    std::cout << "  Mean: " << std::fixed << std::setprecision(2) << summary.ttft_mean() << " ms" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-m" || arg == "--model") {
            ...
            if (i + 1 < argc) config.n_threads = std::stoi(argv[++i]);
