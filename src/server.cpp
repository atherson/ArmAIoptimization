...
bool AIServer::start(const std::string &model_path, int port, int n_ctx,
  ...
  server_thread_ =
      std::thread([this, port]() { server_->listen("0.0.0.0", port); });
...
void AIServer::handle_chat_completions(const httplib::Request &req,
  ...
  try {
    ...
    if (request.stream) {
      ...
      // Engine handles its own locking internally now.
...
ChatRequest AIServer::chat_request_from_json(const json &j) {
  ...
  if (j.contains("messages") && j["messages"].is_array()) {
    for (const auto &msg : j["messages"]) {
      // Protect against malformed objects crashing the parser
      if (!msg.is_object())
        continue;
...
void AIServer::send_sse_stream(httplib::Response &res,
  ...
  res.set_content_provider(
      ...
      [this, request](size_t /*offset*/, httplib::DataSink &sink) -> bool {
        // Engine handles synchronization internally during streaming inference.
        // The network sink writes synchronously which can still block the
        // engine if latency is high, but the application will not segfault.
        engine_->chat_completion_stream(
            request, [&sink](const std::string &token, bool /*is_first*/,
